          ...
          <div className="px-6 sm:px-8 py-8 space-y-8">
            <section className="grid gap-4 sm:gap-6 md:grid-cols-2">
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-canvas dark:bg-canvasDark/80 p-5 sm:p-6">
                <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-ink/60 dark:text-text/70">Program Position</h2>
                <p className="mt-3 text-lg font-medium text-ink dark:text-text">{position}</p>
              </div>
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-canvas dark:bg-canvasDark/80 p-5 sm:p-6 space-y-4">
                {estimatedTime && (
                  <p className="flex items-center text-sm sm:text-base text-ink/80 dark:text-text/80">
                    <Clock className="mr-3 h-5 w-5 text-indigo-500" aria-hidden="true" />
                    <span>
                      <span className="font-semibold text-ink dark:text-text">Estimated Time:</span> {estimatedTime}
                    </span>
                  </p>
                )}
                {prerequisites && (
                  <p className="flex items-center text-sm sm:text-base text-ink/80 dark:text-text/80">
                    ...
                    <span>
                      <span className="font-semibold text-ink dark:text-text">Prerequisites:</span> {prerequisites}
                    </span>
                  </p>
                )}
              </div>
            ...
            <section className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-slate-950 px-6 py-8">
              <h2 className="text-lg font-semibold text-ink dark:text-text">What to Expect</h2>
              <ul className="mt-4 list-disc space-y-2 pl-5 text-ink/80 dark:text-text/80">
