export default function Hero({ label, title, emphasis, deck, meta }: { label: string; title: string; emphasis: string; deck: string; meta: { label: string; value: string }[] }) {
  ...
  const hasEmphasisInTitle = emphasis.length > 0 && emphasisIndex !== -1;
  const before = hasEmphasisInTitle ? title.slice(0, emphasisIndex) : title;
  const after = hasEmphasisInTitle ? title.slice(emphasisIndex + emphasis.length) : "";
  ...
  return (
    <div className="mx-auto max-w-[860px] border-b border-border px-5 pb-14 pt-16 md:px-8 md:pt-20">
      ...
      <h1 className="mt-5 font-mfa-display text-[clamp(42px,6vw,72px)] font-light leading-[1.08] tracking-tight animate-[fadeUp_.7s_.2s_forwards] opacity-0 motion-reduce:opacity-100">
        {hasEmphasisInTitle ? (
          <>
            {before}
            <em className="text-[rgb(var(--mfa-rust))]">{emphasis}</em>
            {after}
          </>
        ) : (
          title
